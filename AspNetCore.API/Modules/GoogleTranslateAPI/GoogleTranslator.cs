...
using GoogleTranslateAPI.TranslationData;
...
namespace GoogleTranslateAPI
{
    ...
    public class GoogleTranslator : ITranslator
    {
        ...
        static GoogleTranslator()
        {
            ...
            Stream stream = assembly.GetManifestResourceStream("GoogleTranslateAPI.Languages.json");
        ...
        protected virtual TranslationResult ResponseToTranslateResultParse(string result, string sourceText, Language sourceLanguage, Language targetLanguage, bool additionInfo)
        {
            ...
            // EXAMPLES
            translationResult.Examples = tmp.Count() >= 14
                 ? GetExample(tmp[13])
                 : null;
        ...
        protected static string[] GetExample(JToken response)
        {
            var result = new List<string>();
            if (response.HasValues)
            {
                var l1 = response[0];
                foreach (var item in l1.Children())
                {
                    try
                    {
                        result.Add(item.First.ToString().Replace("<b>", "").Replace("</b>", ""));
                    }
                    catch
                    {
                        continue;
